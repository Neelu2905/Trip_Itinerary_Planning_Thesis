...
if city!="Select a City":
    ...
    # st.write("utility sorted now")
    # st.write(utility)
    
    # st.write(utility.iloc[0])
    # st.write(utility.iloc[1])
    ...
    # Convert tuple keys to strings for display purposes
    # travel_times_walking_str = {f"{k[0]} → {k[1]}": v for k, v in travel_times_walking.items()}
    # travel_times_taxi_str = {f"{k[0]} → {k[1]}": v for k, v in travel_times_taxi.items()}
    
    # st.write("Walking Travel Times (in minutes):", travel_times_walking_str)
    # st.write("Taxi Travel Times (in minutes):", travel_times_taxi_str)
    ...
    if st.button("GENERATE ITINERARY"):
        ...
        # for i in poi_ids:
        #     if i<len(utility):
        #         st.write(f"{i} = {utility.loc[i, 'poiName']} = {utility.loc[i, 'theme']}")
        ...
        for theme, (lower_bound, upper_bound) in theme_bounds.items():
            ...
            theme_count = quicksum(y[i] for i in poi_ids if i < len(utility) and utility.iloc[i]["theme"] == theme)
        ...
        # for i in poi_ids:
        #     for j in poi_ids:
        #         model.addConstr(w[i,j]==0,name="WalkNotAllowed")
                
        # #mode selection test
        # for i in poi_ids:
        #     for j in poi_ids:
        #         model.addConstr(x[i,j]==0,name="TaxiNotAllowed")
