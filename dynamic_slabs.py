...
if city!="Select a City":
    ...
    def run_optimizer(source_poi, remaining_time_budget, remaining_cost_budget):
        ...
        model.setObjective(quicksum(effective_utility[i] for i in poi_ids if i not in st.session_state.visited_pois), GRB.MAXIMIZE)
            ...
            for j in poi_ids:
                if i != j and (j, i) not in st.session_state.visited_edges:
                    model.addConstr(
                        ...
                        arrival_time[i] >= (
                            arrival_time[j] + (visit_times[j] * ppoi[j]) +
                            (travel_times_walking.get((j, i), 0) * w[j, i] + travel_times_taxi.get((j, i), 0) * x[j, i])
                        ) * z[j, i],
                        name=f"ArrivalTime_{j}_to_{i}"
                    )
        ...
        for i in poi_ids:
            ...
            model.addConstr(arrival_time[i] + (visit_times[i]*ppoi[i]) <= closing_times_fun[i], name=f"ClosingTime_{i}")  # Must leave before closing
        ...
        if model.status == GRB.OPTIMAL and model.objVal > 0:
            ...
            #     st.session_state.total_utility = st.session_state.total_utility+(effective_utility[source_poi].X)
            ...
            if st.session_state.visited_pois:
                ...
                visit_time_estimate = visit_times.get(last_visited_poi, 0) * ppoi[last_visited_poi].X
                # visit_time_taken = math.ceil(random.uniform(0.85 * visit_time_estimate, 1.15 * visit_time_estimate))
                # while visit_times.get(last_visited_poi, 0)!=0 or (visit_time_taken / visit_times.get(last_visited_poi, 0)) < 0.5:
                #     visit_time_taken = math.ceil(random.uniform(0.85 * visit_time_estimate, 1.15 * visit_time_estimate))
                
                while True:
                    visit_time_taken = math.ceil(random.uniform(0.85 * visit_time_estimate, 1.15 * visit_time_estimate))
                    if (visit_times.get(last_visited_poi, 0)==0 or visit_time_taken / visit_times.get(last_visited_poi, 0)) >= 0.5:
                        break

                if source_poi!=0 and source_poi!=ending_poi:
                    ratio = visit_time_taken/visit_times.get(source_poi, 0) # raio of time spent to avg visiting time
                    ...
                    st.session_state.total_utility = st.session_state.total_utility + (utility_scores.get(source_poi,0)*s)
        ...
        if model.status == GRB.OPTIMAL and model.objVal>0:
            ...
            total_entrance_fee_cost = sum(
                ...
                if i not in st.session_state.visited_pois and i != source_poi and y[i].X > 0
            )
        ...
        if model.status == GRB.OPTIMAL and model.objVal>0:
            ...
            # selected_visit_pois = []
            for i in selected_pois:
                visit_time = visit_times[i] * ppoi[i].X  # Get visit time of POI
                if i!=source_poi:
                    total_visit_time += visit_time  # Add to total visit time
    ...
    # Show ongoing trip planning UI when trip is active and not currently in generation process
        ...
        if st.session_state.current_poi == poi_ids[-1]:
            ...
            if generate_button:
                ...
                st.write(f"Travel karne ka cost: {st.session_state.temp_cost}")
    ...
    if st.session_state.end_trip_clicked or st.button("End Trip"):
        ...
        if len(st.session_state.temp_list) != 0:
            ...
            for edge in st.session_state.visited_edges:
                ...
                # Calculate unit direction vector and offset
                dx, dy = lon2 - lon1, lat2 - lat1
                dist = np.hypot(dx, dy)
                offset = 0.002  # adjust if needed (0.001–0.005 works well)

                # Shift points slightly along the direction to avoid overlap
                start_lon = lon1 + dx * offset / dist
                start_lat = lat1 + dy * offset / dist
                end_lon = lon2 - dx * offset / dist
                end_lat = lat2 - dy * offset / dist
