...
if city!="Select a City":
    ...
    def run_optimizer(source_poi, remaining_time_budget, remaining_cost_budget):
        ...
        model.setObjective(quicksum(utility_scores[i] * ppoi[i] for i in poi_ids if i not in st.session_state.visited_pois), GRB.MAXIMIZE)
        ...
        if model.status == GRB.OPTIMAL and model.objVal > 0:
            ...
            #     st.session_state.total_utility = st.session_state.total_utility+(utility_scores[source_poi]*ppoi[source_poi].X)
            ...
            if st.session_state.visited_pois:
                ...
                while True:
                    ...
                    if (visit_times.get(last_visited_poi, 0)==0 or visit_time_taken / visit_times.get(last_visited_poi, 0)) >= 0.5:
                        break
                if source_poi!=0 and source_poi!=ending_poi:
                    ratio = visit_time_taken/visit_times.get(source_poi, 0) # ratio of time spent to avg visiting time
                    if ratio>1:
                        ratio = 1
                    st.write(f"ratio of {source_poi}: {ratio}")
                    st.session_state.total_utility = st.session_state.total_utility + (utility_scores.get(source_poi,0)*ratio)
                    st.write(f"Utility added of {source_poi}: {utility_scores.get(source_poi,0)*ratio}")
                ...
                if last_edge:
                    ...
                    st.write(f"{source_poi} ki entrance fee hai {entrance_fee}")
    ...
    # Show ongoing trip planning UI when trip is active and not currently in generation process
        ...
        if st.session_state.current_poi == poi_ids[-1]:
            ...
            if generate_button:
                ...
                st.write(f"Travel karne ka cost: {st.session_state.temp_cost} juda hai")
...
if st.session_state.trip_active:
    if st.session_state.end_trip_clicked or st.button("End Trip"):
        ...
        if st.session_state.end_trip_clicked and len(st.session_state.temp_list) != 0:
            ...
            st.session_state.total_cost -= st.session_state.temp_cost
            st.write(f"itna cost subtract hua {st.session_state.temp_cost}")
