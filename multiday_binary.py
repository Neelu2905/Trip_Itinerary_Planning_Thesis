...
if city!="Select a City":
    ...
    if st.button("GENERATE ITINERARY"):
        ...
        model.setObjective(quicksum(utility_scores[i] * y[i,d] for i in poi_ids for d in days), GRB.MAXIMIZE) 
        ...
        for d in days:
            ...
            model.addConstr(
                ...
                quicksum(visit_times[i] * y[i,d] for i in poi_ids) <= day_budget,
                name="TimeConstraint"
            )
        ...
        for (a, b) in ordering_constraints:
            for d in days:
                model.addConstr(
                    arrival_time[a, d] + visit_times[a] + (
            ...
            for i in poi_ids:
                for j in poi_ids:
                    if i != j and i != start_node:
                        ...
                        Mji = day_budget + visit_times[j] + (travel_times_walking.get((j,i),0) + travel_times_taxi.get((j,i),0))

                        model.addConstr(
                            arrival_time[i, d]
                            >= arrival_time[j, d] + visit_times[j] + tt
                                - Mji * (1 - z[j, i, d]),
        ...
        for d in days:
            for i in poi_ids:
                ...
                model.addConstr(arrival_time[i, d] <= closing_times[i] - visit_times[i], name=f"ClosingTime_{i}")
        ...
        if model.status == GRB.OPTIMAL and model.objVal > 0:
            ...
            for d in days:
                ...
                selected_pois = [i for i in poi_ids if y[i, d].x > 0.5]
                day_utility = sum(utility_scores[i] for i in selected_pois)
                ...
                total_visit_time_on_this_day = sum(visit_times[i] * y[i, d].X for i in poi_ids)
